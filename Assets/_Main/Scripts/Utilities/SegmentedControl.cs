...
namespace UnityEngine.UI.Extensions {
	...
	public class Segment :
		...
		internal void TransitionButton(bool instant) {
			...
			IssueStatusType statusType = (IssueStatusType) index;
			Color tintColor = selected ? (Color) IssueStatus.StatusTypeToColor(statusType) : button.colors.normalColor;
			//Color textColor = selected ? button.colors.normalColor : this.textColor;
			Color textColor = selected ? IssueStatus.StatusTypeToTextColor(statusType) : this.textColor;
			/// modified section over ----
			...
			switch (button.transition) {
				case Selectable.Transition.ColorTint:
					StartColorTween((Color) tintColor * button.colors.colorMultiplier, instant);
		...
		internal void StoreTextColor() {
			...
			if (!text) {
				var tm = GetComponentInChildren<TextMeshProUGUI>();
				if (!tm)
					return;
